...
- `upload/system/kotlyars_catalog_foundation.ocmod.xml`
...
3. Extend the admin product form through `upload/system/kotlyars_catalog_foundation.ocmod.xml` to save `product_type`, `category_code`, and governed attribute values.
...
- Save / load integration:
  `upload/system/kotlyars_catalog_foundation.ocmod.xml` wires product add/edit/getForm/validateForm without touching core files on disk.
...
- The OCMOD must be installed/refreshed in OpenCart admin before the product form tab appears.

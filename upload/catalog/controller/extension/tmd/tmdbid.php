<?php
class ControllerExtensionTmdTmdBid extends Controller {
	public function index() {
		...
		$status = $this->config->get('tmdauction_status');

		if ($status) {
			$this->load->language('extension/tmd/tmdbid');
			...
			$data['maximum_quantitys'] 	= $this->config->get('tmdauction_maximum_quantity');
			...
			$normalbildlisttext 	= $auctionsetlables[$this->config->get('config_language_id')]['normalbildlisttext'];
			if(!empty($normalbildlisttext)){
			...
			if($data['module_status']==1) {
				...
				$height = $this->config->get('tmdauction_height');
				...
				$width = $this->config->get('tmdauction_width');
				...
				if(!empty($tmdauction_banner)){
					$data['thumbheaderlogo'] = $this->model_tool_image->resize($tmdauction_banner, $height, $width);
				}else{
					$data['thumbheaderlogo'] =$this->model_tool_image->resize('no_image.png', $height, $width);
				}
		...
		$auction_info = $this->model_extension_tmd_tmdmywinningbid->getTmdAuctionProduct($product_id);

		

		$data['total'] 		= $this->model_extension_tmd_tmdauction->getTmdTotalBid($product_id);
		$max_bid 			= $this->model_extension_tmd_tmdauction->getTmdMaxAuctionBid($product_id);
		$this->load->model('extension/tmd/tmdmywinningbid');
		$product_info = $this->model_extension_tmd_tmdmywinningbid->getTmdAuctionProduct($product_id);
		if(isset($max_bid)){
		...
		$win_info = $this->model_extension_tmd_tmdmybid->getTmdAuctionWinner($product_id);
		if(isset($win_info['product_id'])){
			...
			$data['customer_win_status']=false;
			if(!empty($win_info['customer_id'])){
			$data['customer_win_status']=true;
				if($win_info['customer_id'] == $this->customer->getId()){
					$data['customer_win_status']=false;
				}
			}
		
		$data['tmdmybid']=str_replace('&amp;','&',$this->url->link('extension/tmd/tmdbid/bidPages&product_id='.$product_id.''));
			return $this->load->view('extension/tmd/tmdbid', $data);
		}
	}
	...
	public function tmdTimestamp() {
		...
		$this->load->model('extension/tmd/tmdauction');
		$auction_info = $this->model_extension_tmd_tmdauction->getTmdAuction($auction_id);
	...
	public function bidPages() {
		...
		$bidtotal_info 		= $this->model_extension_tmd_tmdauction->getTotalBid($filter_data,$product_id);
		$bid_info 			= $this->model_extension_tmd_tmdauction->getTmdAuctionBids($filter_data,$product_id);
		...
		$pagination->url 	= $this->url->link('extension/tmd/tmdbid/bidPages&product_id='.$product_id,'page={page}');
		...
		return $this->response->setOutput($this->load->view('extension/tmd/tmdbidpage', $data));;
	}
	...
	public function addBid() {
		...
		if (!$this->customer->isLogged()) {
			...
			$min_amount_info = $this->model_extension_tmd_tmdauction->getTmdAuctionProduct($product_id);
			...
			$last_amount 	= $this->model_extension_tmd_tmdauction->getTmdMaxAuctionBid($product_id);
			if(isset($last_amount)){
				$max_bid = $this->currency->format($last_amount,$this->session->data['currency']);
			}else{
				$max_bid = $this->currency->format($min_amount_info['starting_bids'],$this->session->data['currency']);
			}
			if(!empty($this->request->post['bid_amount']))
			{
				$currency_into=$this->db->query("select * from " . DB_PREFIX . "currency  where code = '" . $this->db->escape($this->session->data['currency']) . "'");
				 $this->request->post['bid_amount']=$this->currency->convert($this->request->post['bid_amount'],$this->session->data['currency'],$this->config->get('config_currency'));
			...
			if($max_bid >= $this->request->post['bid_amount'] ){
				...
				$bid_id = $this->model_extension_tmd_tmdauction->AddTmdAuctionBid($product_id, $this->request->post);

				$bid_info = $this->model_extension_tmd_tmdauction->getTmdAuctionBid($bid_id);
				$this->load->model('extension/tmd/tmdmywinningbid');
				$product_info = $this->model_extension_tmd_tmdmywinningbid->getTmdAuctionProduct($product_id);
				...
				$json['total'] 		= $this->model_extension_tmd_tmdauction->getTmdTotalBid($product_id);

				if($this->request->post['bid_amount'] >= $min_amount_info['max_price'] && $min_amount_info['auto_finish'] == 1){
					$this->load->model('extension/tmd/tmdmywinningbid');
					$customer_info 	= $this->model_extension_tmd_tmdauction->addTmdAuctionWinner($bid_id);
